...
resetcron() {
  touch $CRON_FILE
  sed -i '/subscribe.sh/d' $CRON_FILE 2>/dev/null
  [ "$(uci -q get daed-next.config.subscribe_auto_update)" -eq 1 ] && echo "${RANDOM_NUM} */${HOURS} */${DAYS} * * /etc/daed-next/subscribe.sh >/dev/null 2>&1" >>$CRON_FILE
  crontab $CRON_FILE
}
